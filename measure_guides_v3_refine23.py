...
def _ridge_rows(union: np.ndarray, corollas: list[dict], top: int, channels) -> list[dict]:
    ...
    for corolla in corollas:
        cs, _ = cv2.findContours(np.asarray(corolla['mask'], np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        contours.append(max(cs, key=cv2.contourArea) if cs else np.zeros((0, 1, 2), np.int32))

    output: list[dict] = []
    for label in range(1, n):
        left = int(stats[label, cv2.CC_STAT_LEFT]); upper = int(stats[label, cv2.CC_STAT_TOP])
        width = int(stats[label, cv2.CC_STAT_WIDTH]); height = int(stats[label, cv2.CC_STAT_HEIGHT])
        ...
        if not points:
            continue
        centre_x = float(np.mean([p[0] for p in points])); centre_y = float(np.mean([p[1] for p in points]))
...
def detect_organs(union: np.ndarray, corollas: list[dict], top: int, channels) -> list[dict]:
    ...
    existing_points = [(float(r['cx']), float(r['cy'])) for r in existing if r.get('cx') not in (None, '')]
    next_instance = max([int(r.get('organ_instance_id', 0) or 0) for r in existing] + [0]) + 1
    accepted: list[dict] = []
